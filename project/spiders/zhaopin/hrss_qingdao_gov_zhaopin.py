...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(BaseSpiderObject):
    ...
    name = "hrss_qingdao_gov_zhaopin"
    start_urls = 'https://hrss.qingdao.gov.cn/ztzl_47/{type}/zpjz_47/mindex.shtml'
    next_base_urls = 'https://hrss.qingdao.gov.cn/ztzl_47/{type}/zpjz_47/mindex_{page}.shtml'
    contents_base_urls = ''  # 用于拼接详情页网址
    province = "山东省"  # 必填，爬虫省份
    city = ""  # 必填，爬虫城市
    county = ""  # 选填，爬虫区/县
    site_name = '青岛市人力资源和社会保障局'
    source = 'hrss.qingdao.gov.cn'

    timeRange = 7
    max_page = 1

    detail_xpath = '//div[@class="wencon"]'
    ...
    def parse(self, response):
        ...
        node_list  = response.xpath('//div[@class="listChangeDiv"]/ul/li')
 
        for node in node_list:

            baseItem = self.get_base_item()
            baseItem['title'] = node.xpath('.//a/@title').extract_first().strip()
            baseItem['publish_time'] = self.format_time_to_str(node.xpath('.//em/text()').extract_first().strip())
