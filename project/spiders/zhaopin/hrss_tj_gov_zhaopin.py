...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(BaseSpiderObject):
    ...
    name = "hrss_tj_gov_zhaopin"
    start_urls = 'https://hrss.tj.gov.cn/xinwenzixun/{type}/index.html'
    next_base_urls = 'https://hrss.tj.gov.cn/xinwenzixun/{type}/index_{page}.html'
    contents_base_urls = ''  # 用于拼接详情页网址
    province = "天津"  # 必填，爬虫省份
    city = ""  # 必填，爬虫城市
    county = ""  # 选填，爬虫区/县
    site_name = '天津人力资源和社会保障厅'
    source = 'hrss.tj.gov.cn'
    ...
    def parse(self, response):
        ...
        node_list  = response.xpath('//div[@class="fmultimedia-y fl"]/ul/li')
 
        for node in node_list:

            baseItem = self.get_base_item()
            baseItem['title'] = node.xpath('./a/@title').extract_first().strip()
            baseItem['publish_time'] = self.format_time_to_str(node.xpath('.//span[@class="fr"]/text()').extract_first().strip())
