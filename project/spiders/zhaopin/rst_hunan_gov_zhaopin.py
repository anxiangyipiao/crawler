...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(BaseSpiderObject):
    ...
    name = "rst_hunan_gov_zhaopin"
    start_urls = 'https://rst.hunan.gov.cn/rst/xxgk/zpzl/{type}/index.html'
    next_base_urls = 'https://rst.hunan.gov.cn/rst/xxgk/zpzl/{type}/index_{page}.html'
    contents_base_urls = ''  # 用于拼接详情页网址
    province = "湖南省"  # 必填，爬虫省份
    city = ""  # 必填，爬虫城市
    county = ""  # 选填，爬虫区/县
    site_name = '湖南省人力资源和社会保障厅'
    source = 'rst.hunan.gov.cn'

    timeRange = 7
    max_page = 1

    detail_xpath = '//div[@class="pagecont"]'
    ...
    def parse(self, response):
        ...
        node_list  = response.xpath('//tbody[@class="table_list_st"]/tr')
 
        for node in node_list:

            baseItem = self.get_base_item()
            baseItem['title'] = node.xpath('.//a/@title').extract_first().strip()
            baseItem['publish_time'] = self.format_time_to_str(node.xpath('./td[3]/text()').extract_first().strip())
