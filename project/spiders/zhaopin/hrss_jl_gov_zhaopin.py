...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(BaseSpiderObject):
    ...
    name = "hrss_jl_gov_zhaopin"
    start_urls = 'http://hrss.jl.gov.cn/rsrc/{type}/index.html'
    next_base_urls = 'http://hrss.jl.gov.cn/rsrc/{type}/index_{page}.html'
    contents_base_urls = ''  # 用于拼接详情页网址
    province = "吉林省"  # 必填，爬虫省份
    city = ""  # 必填，爬虫城市
    county = ""  # 选填，爬虫区/县
    site_name = '吉林省人力资源和社会保障厅'
    source = 'hrss.jl.gov.cn'


    timeRange = 7
    max_page = 1

    detail_xpath = '//div[@class="meng_biankuang"]'
    ...
    def parse(self, response):
        ...
        node_list  = response.xpath('//div[@class="news_list4 navBlock"]/ul/li')
 
        for node in node_list:

            baseItem = self.get_base_item()
            baseItem['title'] = node.xpath('./a[2]/@title').extract_first().strip()
            baseItem['publish_time'] = self.format_time_to_str(node.xpath('./span/text()').extract_first().strip())
            baseItem['url'] = urljoin(response.url,node.xpath('./a[2]/@href').extract_first().strip())
