...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(BaseSpiderObject):
    ...
    name = "lnrsks_gov_zhaopin"
    start_urls = 'https://www.lnrsks.com/html/{type}/'
    ...
    county = ""  # 选填，爬虫区/县
    site_name = '辽宁人事考试网'
    source = 'www.lnrsks.com'

    timeRange = 7
    max_page = 1

    detail_xpath = '//div[@class="listleftback"]'
    ...
    lis = ['gwy_zhaokaogonggao','linxuangonggao','sydw_zhaopingonggao']
    ...
    def parse(self, response):
        ...
        # 提取数据
        node_list = response.xpath('//div[@class="listleftback"]/div[@class="kaoshilist"]')
        ...
        for node in node_list:
            ...
            item_time = node.xpath('.//div[@class="kaoshilistleftbottom1"]/text()').extract_first().strip() + '-' + node.xpath('.//div[@class="kaoshilistlefttop1"]/text()').extract_first().strip()

            baseItem['publish_time'] = self.format_time_to_str(item_time)
