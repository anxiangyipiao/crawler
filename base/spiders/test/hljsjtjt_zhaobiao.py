...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(ZhaotoubiaoBaseSpider):
    ...
    name = "hljsjtjt_zhaobiao"
    start_urls = 'https://zc.hljsjtjt.com/api/portal/bidnotice/list?keyName=&menuCode=&methodType=&pType=&pubtime=&typeCode=CGGG&page={page}&pageSize=10'
    
    next_base_urls = ''  # 用于下一页网址拼接
    contents_base_urls = 'https://zc.hljsjtjt.com/#/details.html?businessId={id}&activeId=3'  # 用于拼接详情页网址
    page_urls = ""  # 用于获取下一页网址

    province = ""  # 必填，爬虫省份
    city = ""  # 必填，爬虫城市
    county = ""  # 选填，爬虫区/县
    site_name = '黑龙江政企招采数字服务平台'
    source = 'zc.hljsjtjt.com'
    max_page = 20
    ...
    def parse(self, response):
        ...
        for node in node_list:
            ...
            item['publish_time'] = node['pubtime'][:10]
            ...
            if add_task == False:
                ...
                next_url = 'https://zc.hljsjtjt.com/api/portal/details/'+ node['id']
                yield scrapy.Request(next_url, callback=self.parse_detail, meta={'item': item}, headers=self.headers)
        # -------------------------------tag-------------------------------------------
        # 检查当页数据日期，判断是否翻页
