...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(ZhaotoubiaoBaseSpider):
    ...
    name = "wuxi_zcygov_zhaobiao"
    start_urls = 'https://cz.wuxi.zcygov.cn/portal/category'
    
    next_base_urls = ''  # 用于下一页网址拼接
    contents_base_urls = 'https://cz.wuxi.zcygov.cn/portal/detail?articleId={articleId}&parentId={parentId}'  # 用于拼接详情页网址
    page_urls = ""  # 用于获取下一页网址
    ...
    def parse(self, response):
        ...
        node_list  = response.json()['result']['data']['data']
        ...
        for node in node_list:
            ...
            item['publish_time'] = datetime.datetime.fromtimestamp(node['publishDate'] / 1000).strftime('%Y-%m-%d')
            item['url'] = self.contents_base_urls.format(articleId=node['articleId'],parentId=node['parentId'])
    ...
    def parse_detail(self, response):
        ...
        item['contents'] = response.json()['result']['data']['content']
        yield item
