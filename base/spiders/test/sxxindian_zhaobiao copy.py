...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(ZhaotoubiaoBaseSpider):
    ...
    name = "sxxindian_qiye_zhaobiao"
    start_urls = 'http://www.sxxindian.com/EpointWebBuilder/searchlistAction_Custom.action?cmd=getSerachgglxlist'
        
    
    next_base_urls = ''  # 用于下一页网址拼接
    contents_base_urls = 'http://www.sxxindian.com'  # 用于拼接详情页网址
    page_urls = ""  # 用于获取下一页网址

    province = ""  # 必填，爬虫省份
    city = ""  # 必填，爬虫城市
    county = ""  # 选填，爬虫区/县
    site_name = '山西新点招投标交易平台'
    source = 'www.sxxindian.com'
    max_page = 10
    ...
    def start_requests(self):
            ...
            yield scrapy.FormRequest(self.start_urls,method='post',callback=self.parse,dont_filter=True,formdata=self.data,headers=self.headers,meta={'page':0})

    def parse(self, response):
        
        page = response.meta['page']

        node_list  = response.json()['custom']

        node_list = json.loads(node_list)['Table']
        ...
        for node in node_list:
            ...
            item['publish_time'] = node['date']
            item['url'] = self.contents_base_urls + node['href']
        ...
        if page < int(self.max_page) and check_publish_time:
            time.sleep(1)
            page += 1
            self.data['pageIndex'] = str(page)
            yield scrapy.FormRequest(self.start_urls,method='post',callback=self.parse,dont_filter=True,formdata=self.data,headers=self.headers,meta={'page':page})
        else:
            # -------------------------------tag-------------------------------------------
            # 标记翻页结束
            self.page_over = True
