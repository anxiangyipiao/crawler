...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(ZhaotoubiaoBaseSpider):
    ...
    name = "chinapost_jituanzhishu_zhaobiao"
    start_urls = [
        'https://www.chinapost.com.cn/html1/category/181313/7360-{page}.htm',
        'https://www.chinapost.com.cn/html1/category/181313/7359-{page}.htm',  
        'https://www.chinapost.com.cn/html1/category/181313/7358-{page}.htm',
        'https://www.chinapost.com.cn/html1/category/181313/7357-{page}.htm',
         
        ]
    
    next_base_urls = ''  # 用于下一页网址拼接
    contents_base_urls = 'https://www.chinapost.com.cn'  # 用于拼接详情页网址
    page_urls = ""  # 用于获取下一页网址

    province = ""  # 必填，爬虫省份
    city = ""  # 必填，爬虫城市
    county = ""  # 选填，爬虫区/县
    site_name = '中国邮政集团有限公司'
    ...
    def start_requests(self):
        ...
        for url in self.start_urls:
            ...
            yield scrapy.Request(urls,method='get',callback=self.parse,dont_filter=True,meta={'page':0,"param":url},headers=self.headers)
    ...
    def parse(self, response):
        ...
        node_list  = response.xpath('//div[@class="new_list"]/ul/li')[:-1]
        ...
        for node in node_list:
            ...
            item['title'] = node.xpath('./span[1]/a/@title').extract_first().strip()
            item['publish_time'] = node.xpath('./span[2]/text()').extract_first().strip()
            item['url'] = self.contents_base_urls + node.xpath('./span[1]/a/@href').extract_first().strip()
