...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(ZhaotoubiaoBaseSpider):
    ...
    name = "zgdczb_zhaobiao"
    start_urls = [
        'http://www.zgdczb.com/news/shebeizb/{page}.html',
        'http://www.zgdczb.com/news/gongcheng/{page}.html',
    ]
    ...
    def start_requests(self):
        ...
        for url in self.start_urls:
            ...
            yield scrapy.Request(urls,method='get',callback=self.parse,dont_filter=True,meta={'page':1,'param':url},headers=self.headers)
    ...
    def parse(self, response):
        ...
        node_list  = response.xpath('//div[@class="catlist"]/ul/li')
 
        for node in node_list:

            if node.xpath('./a/text()').extract_first() == None:
                continue
            ...
            item['publish_time'] = node.xpath('./i/text()').extract_first().strip()
            item['url'] = self.contents_base_urls + node.xpath('./a/@href').extract_first()
        ...
        if page < int(self.max_page) and check_publish_time:
            ...
            yield scrapy.Request(urls,method='get',callback=self.parse,dont_filter=True,meta={'page':page,'param':param})
        else:
            # -------------------------------tag-------------------------------------------
            # 标记翻页结束
            self.page_over = True
