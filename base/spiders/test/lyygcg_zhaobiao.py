...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(ZhaotoubiaoBaseSpider):
    ...
    name = "lyygcg_zhaobiao"
    start_urls = 'https://www.lyygcg.com/inteligentsearch/rest/esinteligentsearch/getFullTextDataNew'
    
    next_base_urls = ''  # 用于下一页网址拼接
    contents_base_urls = 'https://www.lyygcg.com'  # 用于拼接详情页网址
    page_urls = ""  # 用于获取下一页网址

    province = ""  # 必填，爬虫省份
    city = ""  # 必填，爬虫城市
    county = ""  # 选填，爬虫区/县
    site_name = '临沂市阳光采购服务平台'
    source = 'www.lyygcg.com'
    max_page = 20
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(self.start_urls,method='post',callback=self.parse,dont_filter=True,meta={'page':0},headers=self.headers,body=json.dumps(self.data))
    ...
    def parse(self, response):
        ...
        for node in node_list:
            ...
            item['publish_time'] = node['infodate'][:10]
            item['url'] = self.contents_base_urls + node['linkurl']
        ...
        if page < int(self.max_page) and check_publish_time:
            time.sleep(1)
            page += 1
            self.data['pn'] = page * 10
            yield scrapy.Request(self.start_urls,method='post',callback=self.parse,dont_filter=True,meta={'page':page},headers=self.headers,body=json.dumps(self.data))
        else:
            # -------------------------------tag-------------------------------------------
            # 标记翻页结束
            self.page_over = True
