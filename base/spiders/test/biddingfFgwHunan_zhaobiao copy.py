...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(ZhaotoubiaoBaseSpider):
    ...
    name = "biddingfFgwHunan_zhaobiao"
    start_urls = 'http://bidding.fgw.hunan.gov.cn/ztb/api/getBiddingList?totalCount=42244&limit=10&page={page}&categoryId=88&areaNo='
    
    next_base_urls = ''  # 用于下一页网址拼接
    contents_base_urls = 'http://bidding.fgw.hunan.gov.cn/ztbPdf/'  # 用于拼接详情页网址
    page_urls = ""  # 用于获取下一页网址

    province = ""  # 必填，爬虫省份
    city = ""  # 必填，爬虫城市
    county = ""  # 选填，爬虫区/县
    site_name = '湖南省招标投标交易平台'
    source = 'bidding.fgw.hunan.gov.cn'
    max_page = 10
    ...
    def parse(self, response):
        ...
        node_list  = response.json()['page']['list']
        ...
        for node in node_list:
            ...
            item['title'] = node['noticeTitle']
            item['publish_time'] = node['createTime'][:10]
            item['url'] = self.contents_base_urls + node['bidFilePath']
    ...
    def parse_detail(self, response):
        ...
        item['contents'] = response.json()['Body']
        yield item
