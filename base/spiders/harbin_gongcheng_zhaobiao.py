...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(ZhaotoubiaoBaseSpider):
    ...
    name = "harbin_gongcheng_zhaobiao"
    start_urls = 'https://scm.harbin-electric.com/portal/project_notice_info/list'
    
    next_base_urls = ''  # 用于下一页网址拼接
    contents_base_urls = 'https://scm.harbin-electric.com/portal/central/project_notice_info/detail?id={id}&originalId={originalId}'  # 用于拼接详情页网址
    page_urls = ""  # 用于获取下一页网址

    province = ""  # 必填，爬虫省份
    city = ""  # 必填，爬虫城市
    county = ""  # 选填，爬虫区/县
    site_name = '哈电集团电子商务平台'
    source = 'scm.harbin-electric'
    max_page = 2
    ...
    def parse(self, response):
        ...
        for node in node_list:
            ...
            item['publish_time'] = node['pulishDate'][:10]
            item['url'] = self.contents_base_urls.format(id=node['id'],originalId=node['originalId'])
