...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(ZhaotoubiaoBaseSpider):
    ...
    name = "chinaccsscm_zhaobiao"
    start_urls = 'https://zb.chinaccsscm.cn/zbgg/index_{page}.jhtml'
    
    next_base_urls = ''  # 用于下一页网址拼接
    contents_base_urls = 'https://zb.chinaccsscm.cn'  # 用于拼接详情页网址
    page_urls = ""  # 用于获取下一页网址

    province = ""  # 必填，爬虫省份
    city = ""  # 必填，爬虫城市
    county = ""  # 选填，爬虫区/县
    site_name = '中通服供应链股份有限公司'
    source = 'zb.chinaccsscm.cn'
    max_page = 10
    ...
    def parse(self, response):
        ...
        for node in node_list:
            ...
            item['publish_time'] = node.xpath('./p/span[3]/text()').extract_first().strip().split('：')[1][:10]
            
            if node.xpath('./a/@href').extract_first().strip().startswith('http'):
