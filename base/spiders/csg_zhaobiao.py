...
class Shandong_JiNan_ggzy_jianshegongcheng_zhaobiao(ZhaotoubiaoBaseSpider):
    ...
    name = "csg_zhaobiao"
    start_urls = 'https://www.bidding.csg.cn/zbcg/index_{page}.jhtml'
    
    next_base_urls = ''  # 用于下一页网址拼接
    contents_base_urls = 'https://www.bidding.csg.cn'  # 用于拼接详情页网址
    page_urls = ""  # 用于获取下一页网址
    ...
    def parse(self, response):
        ...
        for node in node_list:
            ...
            item['title'] = node.xpath('./a[2]/text()').extract_first().strip()
            item['publish_time'] = node.xpath('./span[1]/span/text()').extract_first().strip()
            item['url'] = self.contents_base_urls + node.xpath('./a[2]/@href').extract_first().strip()
